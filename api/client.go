...
func NewClient(config Config) (*Client, error) {
	...
	if config.MaxResponseBytes == 0 { config.MaxResponseBytes = 8 << 20 }
	if config.MaxErrorBytes == 0 { config.MaxErrorBytes = 8 << 10 }
	if config.MaxAttempts == 0 { config.MaxAttempts = 3 }
	if config.MaxRetryDelay == 0 { config.MaxRetryDelay = 10 * time.Second }
	...
	hc := http.Client{}
	if config.HTTPClient != nil { hc = *config.HTTPClient }
	hc.Jar = nil
...
func (e *ResponseError) Error() string { return "langfuse api: " + e.Operation + ": HTTP " + strconv.Itoa(e.StatusCode) }
...
func formatError(state fmt.State, verb rune, message string) {
	if verb == 'q' { message = strconv.Quote(message) }
...
func (c *Client) do(ctx context.Context, operation, method, path string, query url.Values, body, result any) error {
	if c == nil || c.http == nil { return ErrUninitialized }
	if ctx == nil { return errors.New("langfuse api: nil context") }
	ctx, cancel := context.WithTimeout(ctx, c.timeout)
	defer cancel()
	if err := ctx.Err(); err != nil { return requestError(operation, "canceled", err) }
	var payload []byte
	if body != nil {
		var err error
		payload, err = json.Marshal(body)
		if err != nil { return requestError(operation, "invalid JSON request", nil) }
		if len(payload) > 8<<20 { return requestError(operation, "request exceeds size limit", nil) }
	}
	endpoint := c.baseURL + "/api/public" + path
	if len(query) > 0 { endpoint += "?" + query.Encode() }
	attempts := 1
	if method == http.MethodGet { attempts = c.maxAttempts }
	for attempt := 0; attempt < attempts; attempt++ {
		req, err := http.NewRequestWithContext(ctx, method, endpoint, bytes.NewReader(payload))
		if err != nil { return requestError(operation, "invalid request", nil) }
		...
		if requestErr != nil {
			...
			if attempt+1 < attempts {
				if err := sleepRetry(ctx, c.retryDelay(attempt, "")); err != nil { return requestError(operation, "canceled", err) }
				continue
			}
			return requestError(operation, "transport failed", requestErr)
		}
		if response == nil || response.Body == nil { return requestError(operation, "invalid response", ErrInvalidResponse) }
		if response.StatusCode < 200 || response.StatusCode >= 300 {
			...
			if attempt+1 < attempts && retryableStatus(response.StatusCode) {
				if err := sleepRetry(ctx, delay); err != nil { return requestError(operation, "canceled", err) }
				continue
			}
			return failure
		}
		...
		_ = response.Body.Close()
		if err := ctx.Err(); err != nil { return requestError(operation, "canceled", err) }
		if readErr != nil { return requestError(operation, "response read failed", readErr) }
		if int64(len(data)) > c.maxResponseBytes { return requestError(operation, "response exceeds size limit", ErrResponseTooLarge) }
		if result == nil { return nil }
		trimmed := bytes.TrimSpace(data)
		if len(trimmed) == 0 || bytes.Equal(trimmed, []byte("null")) { return requestError(operation, "invalid response", ErrInvalidResponse) }
		decoder := json.NewDecoder(bytes.NewReader(data))
		decoder.UseNumber()
		if err := decoder.Decode(result); err != nil { return requestError(operation, "invalid response", ErrInvalidResponse) }
		var trailing any
		if err := decoder.Decode(&trailing); !errors.Is(err, io.EOF) { return requestError(operation, "invalid response", ErrInvalidResponse) }
		return nil
	}
	return requestError(operation, "request attempts exhausted", nil)
}

func requestError(operation, kind string, cause error) error { return &RequestError{Operation: operation, Kind: kind, cause: cause} }
...
func (c *Client) retryDelay(attempt int, header string) time.Duration {
	if seconds, err := strconv.ParseInt(header, 10, 64); err == nil && seconds >= 0 {
		if seconds > int64(c.maxRetryDelay/time.Second) { return c.maxRetryDelay }
...
func escapedSegment(value string) (string, error) {
	...
	if value == "." || value == ".." { return strings.Repeat("%2E", len(value)), nil }

...
func (p *CursorPage[T]) UnmarshalJSON(data []byte) error {
	...
	if len(envelope.Data) == 0 || bytes.Equal(bytes.TrimSpace(envelope.Data), []byte("null")) || len(envelope.Meta) == 0 || bytes.Equal(bytes.TrimSpace(envelope.Meta), []byte("null")) {
		return ErrInvalidResponse
	}
...
func pageValues(options PageOptions) (url.Values, error) {
	if options.Page < 0 || options.Limit < 0 || options.Limit > 100 {
		return nil, errors.New("langfuse api: invalid page or limit")
	}
	if options.Page == 0 {
		options.Page = 1
	}
...
func normalizeWalk(options WalkOptions) (WalkOptions, error) {
	...
	if options.MaxPages < 1 || options.MaxPages > 1000 || options.MaxItems < 1 || options.MaxItems > 100000 || options.PageSize < 1 || options.PageSize > 100 {
		return WalkOptions{}, errors.New("langfuse api: invalid pagination bounds")
	}
	return options, nil
}
...
func WalkPages[T any](ctx context.Context, options WalkOptions, fetch func(context.Context, PageOptions) (Page[T], error), yield func(T) error) error {
	if ctx == nil || fetch == nil || yield == nil {
		return errors.New("langfuse api: nil pagination context or callback")
	}
	...
	for pageNumber := 1; pageNumber <= options.MaxPages; pageNumber++ {
		...
		if page.Meta.Page != pageNumber || page.Meta.TotalPages < 0 || len(page.Data) > options.PageSize {
		...
		if pageNumber >= page.Meta.TotalPages {
			return ctx.Err()
		}
		if count == options.MaxItems {
			return ErrWalkLimit
...
func WalkCursor[T any](ctx context.Context, options WalkOptions, fetch func(context.Context, CursorOptions) (CursorPage[T], error), yield func(T) error) error {
	...
	for range options.MaxPages {
		...
		if len(page.Data) > options.PageSize {
			return ErrPagination
		}
		for _, item := range page.Data {
			if err := ctx.Err(); err != nil {

...
func assertJSONEqual(t *testing.T, got []byte, want string) {
	t.Helper()
	var left, right any
	decode := func(data string, out *any) error {
		decoder := json.NewDecoder(strings.NewReader(data))
		decoder.UseNumber()
		return decoder.Decode(out)
	}
	if err := decode(string(got), &left); err != nil {
		t.Fatalf("invalid actual JSON: %v", err)
	}
	if err := decode(want, &right); err != nil {
...
func TestPromptCreateChatUnionAndEmptyPresence(t *testing.T) {
	...
	if prompt.Type != PromptChat || len(prompt.Prompt.Chat) != 2 || prompt.Prompt.Chat[1].Placeholder.Name != "history" {
		t.Fatalf("decoded prompt=%+v", prompt)
	}
	if !prompt.CommitMessage.Present || !prompt.CommitMessage.Null || !prompt.ResolutionGraph.Present || !prompt.ResolutionGraph.Null {
		t.Fatal("lost explicit nulls")
	}
	if !strings.Contains(string(prompt.Config), "9007199254740993") {
		t.Fatal("rounded arbitrary config")
	}
}
...
func TestPromptCreateTextAndUnionValidation(t *testing.T) {
	...
	for _, invalid := range []CreatePromptRequest{
		...
		if _, err := json.Marshal(invalid); err == nil {
			t.Errorf("accepted invalid prompt request")
		}
	}
	for _, invalid := range []string{`null`, `{}`, `{"role":"user"}`, `{"type":"message","role":"user","content":"x"}`, `{"type":"future","name":"x"}`, `{"name":"x","role":"user","content":"x"}`} {
		...
		if err := json.Unmarshal([]byte(invalid), &entry); err == nil {
			t.Errorf("accepted invalid entry %s", invalid)
		}
	}
}
...
func TestPromptSelectionMismatchAndDeletionSafety(t *testing.T) {
	...
	defer server.Close()
	client := testClient(t, server, nil)
	if _, err := client.Prompts.Get(t.Context(), "p", GetPromptOptions{Version: 2}); !errors.Is(err, ErrInvalidResponse) {
		t.Fatalf("wrong version accepted: %v", err)
	}
	if _, err := client.Prompts.Get(t.Context(), "other", GetPromptOptions{}); !errors.Is(err, ErrInvalidResponse) {
	...
	for _, options := range []DeletePromptOptions{{}, {Version: -1}, {Version: 2, Label: "staging"}, {Version: 2, AllVersions: true}, {Label: "staging", AllVersions: true}} {
		if err := client.Prompts.Delete(t.Context(), "p", options); err == nil {
			t.Fatal("accepted ambiguous deletion")
		}
	}
	if _, err := client.Prompts.Get(t.Context(), "p", GetPromptOptions{Version: 2, Label: "staging"}); err == nil {
		t.Fatal("accepted ambiguous retrieval")
	}
	...
	for _, test := range []struct {
		...
		if err := client.Prompts.Delete(t.Context(), "p", test.options); err != nil {
			t.Fatal(err)
		}
		if got := lastQuery.Load(); got != test.query {
			t.Fatalf("query=%s want=%s", got, test.query)
		}
	}
}
...
func TestPromptLabelUpdateWireAndSingleAttempt(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls.Add(1)
		if r.Method != http.MethodPatch || r.URL.EscapedPath() != "/api/public/v2/prompts/folder%2Fp/versions/2" {
	...
	defer server.Close()
	client := testClient(t, server, nil)
	if _, err := client.Prompts.UpdateLabels(t.Context(), "folder/p", 2, nil); err == nil {
		t.Fatal("accepted failed write")
	}
	if calls.Load() != 1 {
		t.Fatalf("replayed label mutation %d times", calls.Load())
	}
	if _, err := client.Prompts.UpdateLabels(t.Context(), "folder/p", 2, []string{"latest"}); err == nil {
		t.Fatal("accepted reserved latest label")
	}
	if calls.Load() != 1 {
		t.Fatal("invalid label caused I/O")
	}
}
...
func TestPromptListFilterDatesAndPage(t *testing.T) {
	...
	if err != nil || page.Meta.Page != 2 || len(page.Data) != 0 {
...
func TestOptionalAndSparseFieldPreserveNullZeroAndAbsence(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(original), &response); err != nil {
		t.Fatal(err)
	}
	if response.Absent.Present || !response.Null.Null || !response.Zero.Present || response.Zero.Null || response.Zero.Value != 0 || response.Flag.Value {
		t.Fatal("presence semantics changed")
	}
	...
	if !validateNumber(json.Number("0")) || !validateNumber(json.Number("1e1000")) || validateNumber(json.Number("NaN")) || validateNumber(json.Number("")) {
		t.Fatal("number validation changed")
	}
}
...
func TestScoreValueUnionIsLossless(t *testing.T) {
	...
	if _, err := json.Marshal(ScoreValue{}); err == nil {
		t.Fatal("accepted an empty score union")
	}
}
...
func TestWalkPagesEmptyIntermediateAndCancellation(t *testing.T) {
	...
	if err != nil || !reflect.DeepEqual(pages, []int{1, 2, 3}) || !reflect.DeepEqual(items, []int{1, 3}) {
		t.Fatalf("pages=%v items=%v err=%v", pages, items, err)
	}
...
func TestWalkBoundsAndRepeatedCursor(t *testing.T) {
	...
	err := WalkCursor(t.Context(), WalkOptions{}, func(_ context.Context, options CursorOptions) (CursorPage[int], error) {
		calls++
		if calls == 2 && options.Cursor != next {
			t.Fatal("cursor was modified")
		}
	...
	if !errors.Is(err, ErrPagination) || calls != 2 {
	...
	if !errors.Is(err, ErrWalkLimit) || calls != 2 {
		t.Fatalf("bound calls=%d err=%v", calls, err)
	}
	count := 0
	err = WalkPages(t.Context(), WalkOptions{MaxItems: 1}, func(_ context.Context, options PageOptions) (Page[int], error) {
		return Page[int]{Data: []int{1, 2}, Meta: PageMeta{Page: options.Page, TotalPages: 1}}, nil
	}, func(_ int) error { count++; return nil })
	if !errors.Is(err, ErrWalkLimit) || count != 1 {
		t.Fatalf("item bound count=%d err=%v", count, err)
	}
}
...
func TestPaginationRejectsMalformedEnvelopes(t *testing.T) {
	for _, input := range []string{`{}`, `{"data":null,"meta":{"page":1,"limit":1}}`, `{"data":[],"meta":null}`, `{"data":[1,2],"meta":{"page":1,"limit":1,"totalItems":2,"totalPages":2}}`} {
		...
		if err := json.Unmarshal([]byte(input), &page); err == nil {
			t.Fatalf("accepted page %s", input)
		}
	}
	for _, input := range []string{`{}`, `{"data":null,"meta":{"nextCursor":null}}`, `{"data":[],"meta":null}`, `{"data":[]}`} {
		var page CursorPage[int]
		if err := json.Unmarshal([]byte(input), &page); err == nil {
			t.Fatalf("accepted cursor page %s", input)
		}
	}
}

func FuzzPromptContent(f *testing.F) {
	for _, seed := range []string{`"Hi {{name}}"`, `[{"role":"user","content":""}]`, `[{"type":"placeholder","name":"history"}]`, `null`, `[]`} {
		f.Add(seed)
	}
	f.Fuzz(func(t *testing.T, input string) {
		...
		if json.Unmarshal([]byte(input), &content) != nil {
			return
		}
		data, err := json.Marshal(content)
		if err != nil || !json.Valid(data) {
			t.Fatalf("accepted content cannot marshal: %v", err)
		}
...
func FuzzScoreValue(f *testing.F) {
	for _, seed := range []string{`9007199254740993`, `true`, `"label"`, `null`, `[]`} {
		f.Add(seed)
	}
	f.Fuzz(func(t *testing.T, input string) {
		if len(input) > 4096 {
		...
		data, err := json.Marshal(value)
		if err != nil || !json.Valid(data) {
			t.Fatalf("accepted value cannot marshal: %v", err)
		}
	})
}

// Package langfusegenai records Langfuse observations for Gemini API
// calls (Google AI Developer API and Vertex AI) made through any
// HTTP-based client, including google.golang.org/genai, without
// wrapping or modifying that client.
//
// For the Developer API, attach the transport to the client:
//
...
//	})
//
// For Vertex AI, genai uses a caller-supplied HTTPClient as-is, so the
// authenticated transport must be composed explicitly and the Langfuse
// transport layered outside it; the module README shows the complete
// composition built on cloud.google.com/go/auth/httptransport, which
// preserves the caller's client policy and keeps Authorization headers
...
const adapterMarker = "langfusegenai"
...
// proxied endpoints the host classifier cannot name. Invalid values
...
// route information (including the model), HTTP status, and timing are
// recorded.
...
// a context. Wire-derived facts always win for model, usage, timing,
// and status; CallAttributes win for name, prompt link, and the
// metadata keys they set.
...
// observation per recognized Gemini-wire HTTP attempt passing through
...
// the langfuseopenai adapter is supported; each passes unrecognized
// routes through.

...
Only one active Langfuse client is supported on a borrowed provider. A
...
At graceful shutdown, stop Langfuse before the application-owned provider:
...
Langfuse context for provider shutdown can leave the provider no time if the

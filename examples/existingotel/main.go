...
func run(ctx context.Context) error {
	...
	if err != nil {
		...
		return fmt.Errorf("attach Langfuse: %w", err)
	}

	workErr := handleRequest(ctx, provider, lf)

	// End application work first, then unregister and stop Langfuse's processor,
...
func handleRequest(ctx context.Context, provider *sdktrace.TracerProvider, lf *langfuse.Client) error {
	...
	generation.Update(langfuse.ObservationAttributes{
		Output: "Langfuse registers one additional processor on the existing provider.",
